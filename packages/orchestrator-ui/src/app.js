...
function initOutputAndFilesActions() {
  ...
  // Native File Open & Diff Buttons
  const btnFileOpenNative = document.getElementById('btn-file-open-native');
  if (btnFileOpenNative) {
    btnFileOpenNative.addEventListener('click', () => {
      if (activeSelectedFilePath && window.OrchestratorBridge) {
        window.OrchestratorBridge.openFile(activeSelectedFilePath);
      }
    });
  }

  const btnFileDiffNative = document.getElementById('btn-file-diff-native');
  if (btnFileDiffNative) {
    btnFileDiffNative.addEventListener('click', () => {
      if (activeSelectedFilePath && window.OrchestratorBridge) {
        window.OrchestratorBridge.openDiff(activeSelectedFilePath, activeSelectedFilePath);
      }
    });
  }
}

function initNativeModeBridge() {
  const btnNative = document.getElementById('btn-native-mode');
  if (btnNative) {
    btnNative.addEventListener('click', () => {
      if (window.OrchestratorBridge) {
        window.OrchestratorBridge.exitUIMode();
      }
    });
  }

  if (window.OrchestratorBridge) {
    window.OrchestratorBridge.onMessage((msg) => {
      if (msg.type === 'WORKSPACE_INFO') {
        const pName = document.getElementById('ws-project-name');
        const bName = document.getElementById('ws-branch-name');
        if (pName && msg.projectName) pName.textContent = msg.projectName;
        if (bName && msg.branch) bName.textContent = msg.branch;
      }
    });

    window.OrchestratorBridge.getWorkspaceInfo();
  }
}
